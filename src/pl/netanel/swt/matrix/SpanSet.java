...
/**
 * Manages a not overlapping set of cell spans.
 *
 * @author Jacek
 * @created 15-11-2010
 */
class SpanSet<X extends Number, Y extends Number> {
  ...
  private Math<Y> mathY;
	...
	public SpanSet(NumberOrder<X> orderX, NumberOrder<Y> orderY) {
		this.orderX = orderX;
    this.orderY = orderY;
    mathX = orderX.math;
    mathY = orderY.math;
	...
	public boolean contains(X indexX, Y indexY) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (indexY != null) {
        containsY = new SpanExtentSequence<Y>(orderY, ey.start.getValue(), ey.end.getValue()).contains(indexY);
			}
			if (indexX != null) {
			  containsX = new SpanExtentSequence<X>(orderX, ex.start.getValue(), ex.end.getValue()).contains(indexX);
			}
	...
	public void add1(X startX, X endX, Y startY, Y endY) {
		insertNew = true;
		int i = 0;
...
//	}
	...
	public SpanSet<X, Y> copy() {
		SpanSet<X, Y> copy = new SpanSet<X, Y>(orderX, orderY);
		int size = size();
		for (int i = 0; i < size; i++) {
