...
public class Painter<X extends Number, Y extends Number> {
  ...
  protected void paint(int x, int y, int width, int height) {
    ...
    paintText();
    _width += imagesWidth;
    _x = java.lang.Math.max(_x, width - imagesWidth - style.imageMarginX);
    if (imagesAfter != null) {
...
//    gc.drawRectangle(_x, _y, _width, _height);
  ...
  private void paintText() {
    if (text == null) return;
    textSize.x = _width - 2 * style.textMarginX;
    ...
    case SWT.CENTER:
      x2 += (_width - textSize.x) / 2; break;
    case SWT.RIGHT: case SWT.END: case SWT.BOTTOM:
      x2 += _width - textSize.x - style.textMarginX; break;
    }
