 ...
 */
class NumberSetCore<N extends Number> implements NumberSet<N> {
...
//	}
	...
	public NumberSetCore(Math<N> math, boolean sorted) {
		this.sorted = sorted;
		this.math = math;
		...
		client = new NumberSetClient<N>(this);
	};
...
//			case BEFORE: 		break;
	...
	public boolean addAll(final NumberSet<N> set) {
	  boolean result = false;
	  if (set instanceof NumberSetCore) {
      for (MutableExtent<N> e: ((NumberSetCore<N>) set).items) {
...
//	  }
	...
	public NumberSetCore<N> copy() {
		NumberSetCore<N> copy = new NumberSetCore<N>(math, sorted);
		for (MutableExtent<N> e: items) {
...
//	  }
  ...
  ExtentSequence<N> extentSequence(Query<N> query) {
    if (query == null) return new ExtentSequence.Forward<N>(this);

    ExtentSequence<N> seq;
    if (query.subtract != null) {
      NumberSetCore<N> copy = NumberSetCore.this.copy();
      copy.removeAll(query.subtract);
      seq = query.backward ?
  ...
  NumberSequence<N> numberSequence(Query<N> query) {
  ...
  public Iterator<Extent<N>> extentIterator(Query<N> query) {
    return extentSequence(query).iterator();
  }

  @Override
  public Iterator<N> numberIterator(Query<N> query) {
    return numberSequence(query).iterator();
  }
