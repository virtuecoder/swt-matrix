...
public class Axis<N extends Number> {
   ...
   * @throws IndexOutOfBoundsException if the headerIndex or bodyIndex is out of
   *           range ( <tt>index &lt; 0 || index &gt;= sectionCount</tt>)
  ...
  public Axis(Class<N> numberClass, int sectionCount, int headerIndex,
    ...
    layout = new AxisLayout<N>(numberClass, sectionCount, headerIndex,
        bodyIndex);
    computeSizeLayout = new AxisLayout<N>(headerIndex, bodyIndex,
        layout.sections);
   ...
   * @throws IndexOutOfBoundsException if sectionIndex is out of 0 ...
   *           {@link #getSectionCount()}-1 bounds
   *
   */
  ...
  public int getViewportItemCount() {
    ...
    return layout.head.cells.size() + layout.tail.cells.size()
        + layout.main.cells.size();
  }
  ...
  /**
   * Returns <code>true</code> if the given item is visible in the viewport,
   * or <code>false</code> otherwise.
   *
   * @param section of the item to check
   * @param index of the item to check
   * @return <code>true</code> if the given item is visible in the viewport
   * @throws IllegalArgumentException if item is <code>null</code> or item's
   *           section does not belong to this axis.
   * @throws IndexOutOfBoundsException if item's index is out of 0 ...
   *           {@link SectionCore#getCount()}-1 bounds
   */
  public boolean isItemInViewport(Section<N> section, N index) {
    AxisItem<N> item = AxisItem.create(section, index);
    checkItem(item, "item");
    return layout.getCache(SectionCore.from(section), index) != null;
  }
   ...
   *
   * @throws IllegalArgumentException if section1 or section2 is
   *           <code>null</code> or does not belong to this axis.
   */
   ...
   * @return comparison result
   *
   * @throws IllegalArgumentException if item1 or item2 is <code>null</code> or
   ...
   * causes the content to scroll automatically and extend the dragged distance.
   *
   * @param offset maximum distance from the edge of the scrolling area in which
   ...
   * axis item width.
   *
   * @param offset maximum distance from the edge of the line in which dragging
