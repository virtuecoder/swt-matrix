 ...
 * and expand) when a drag occurs over a <code>Matrix</code>.
 * <p>
 * Classes that wish to provide their own drag under effect for a
 * <code>Matrix</code> can extend the <code>DropTargetAdapter</code> class and
 * override any applicable methods in <code>DropTargetAdapter</code> to display
 * their own drag under effect.
 * </p>
 ...
 */
public class MatrixDropTargetEffect<X extends Number, Y extends Number> extends DropTargetEffect {
  ...
  private X scrollIndexX;
  private long scrollBeginTime;
//  private Y expandItem;
//  private long expandBeginTime;

  private Cell<X, Y> selectedCell;
  private Cell<X, Y> insertCell;
  private boolean insertBefore;

  /**
   * Creates a new <code>MatrixDropTargetEffect</code> to handle the drag under
   * effect on the specified <code>Matrix</code>.
   * 
   * @param matrix the <code>Matrix</code> over which the user positions the
   *          cursor to drop the data
   */
  public MatrixDropTargetEffect(Matrix<X, Y> matrix) {
    super(matrix);
    this.matrix = matrix;
  }

  /**
   * Set this value to true to make drop feedback in <code>Matrix</code> always
   * behave like the Matrix was <em>not</em> in cell selection mode. The default
   * is false.
   * <p>
   * A value of true, means that for {@link DND#FEEDBACK_SELECT} full rows will
  ...
  int checkEffect(int effect) {
    // Some effects are mutually exclusive. Make sure that only one of the
    // mutually exclusive effects has been specified.
  ...
  public Widget getItem(int x, int y) {
    ...
    coordinates = matrix.toControl(coordinates);
    return matrix.getItem(coordinates);
  }

  /**
   * This implementation of <code>dragEnter</code> provides a default drag under
   * effect for the feedback specified in <code>event.feedback</code>. For
   * additional information see <code>DropTargetAdapter.dragEnter</code>.
...
//    expandItem = null;
  ...
  /**
   * This implementation of <code>dragLeave</code> provides a default drag under
   * effect for the feedback specified in <code>event.feedback</code>. For
   * additional information see <code>DropTargetAdapter.dragLeave</code>.
   * Subclasses that override this method should call
...
//    expandItem = null;
  ...
  /**
   * This implementation of <code>dragOver</code> provides a default drag under
   * effect for the feedback specified in <code>event.feedback</code>. For
   * additional information see <code>DropTargetAdapter.dragOver</code>.
   * Subclasses that override this method should call
  ...
  public void dragOver(DropTargetEvent event) {
    ...
    coordinates = matrix.toControl(coordinates);
    Y hoverItem = matrix.getItem(coordinates);
    Y hoverColumn = matrix.getColumn(coordinates);
    Point hoverCell = matrix.getCell(coordinates);
    ...
    if ((effect & DND.FEEDBACK_SCROLL) == 0) {
      ...
      if (hoverItem != null && scrollIndexX == hoverItem && scrollBeginTime != 0) {
        if (System.currentTimeMillis() >= scrollBeginTime) {
          Y topItem = matrix.getItem(matrix.getTopIndex());
          Y nextItem = hoverItem == topItem ? matrix
            .getPreviousVisibleItem(hoverItem) : matrix
            .getNextVisibleItem(hoverItem);
          boolean scroll = nextItem != null
            && matrix.isInDragScrollArea(coordinates);
          if (scroll) {
            matrix.showItem(nextItem);
          }
          scrollBeginTime = 0;
          scrollIndexX = null;
        }
...
//
//            // Manually fire the expand event, otherwise if it hasn't been
//            // previously
//            // expanded, you'll see the dummy child node instead of actual
//            // contents.
//            // Also, if we don't check hasChildren and just fire the event, then
//            // the
...
//    }
    ...
    if ((effect & DND.FEEDBACK_SELECT) != 0 && hoverCell != null) {
      if (!hoverCell.equals(selectedCell)) {
        if (selectedCell != null) deselect(selectedCell);
        select(hoverCell);
  ...
  private void select(X indexX, Y indexY) {
    if (matrix.getCellSelectionEnabled() && !ignoreCellSelection)
      matrix.selectCell(cell);
    else
      matrix.select(cell.y);

  }

  private void deselect(X indexX, Y indexY) {
    if (matrix.getCellSelectionEnabled() && !ignoreCellSelection)
      matrix.deselectCell(cell);
    else
      matrix.deselect(cell.y);

  }

  private void setInsertMark(X indexX, Y indexY, boolean before) {
    if (cell != null) {
      if (matrix.getCellSelectionEnabled() && !ignoreCellSelection)
        matrix
          .setInsertMark(matrix.getItem(cell.y), matrix.getColumn(cell.x), before);
      else
        matrix.setInsertMark(matrix.getItem(cell.y), null, before);
    }
    else {
      matrix.setInsertMark(null, null, false);
    }
  }
}
