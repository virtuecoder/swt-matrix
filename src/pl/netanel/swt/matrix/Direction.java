...
abstract class Direction<N extends Number> implements Sequence {
	...
	@Override
	public boolean next() {
	  AxisItem<N> nextItem = nextItem();
	  return nextItem != null;
	}


	public boolean initNotEmpty() {
		i = firstSection();
		level = 0;
		return hasMore = pending = nextInternal(null);
	}

	public void init(AxisItem<N> start) {
	  set(start);
	}
	...
	private void set(AxisItem<N> item) {
  ...
  public AxisItem<N> getItem() {
	...
	public AxisItem<N> first() {
		if (!initNotEmpty()) return null;
		return nextItem();
	}

	public AxisItem<N> nextItem(AxisItem<N> item) {
		if (!setHasMore(item)) return null;
		return getItem();
	}
