 ...
 * @author marcin
 */
public class GuiTable_Jacek extends Composite {

  private Matrix<Integer, Integer> matrix;
  private static String[] allCols;
  private static Object[][] values;

  public GuiTable_Jacek(Composite parentComposite, int style) {
  ...
  private void init() {
    ...
    matrix.getAxisX().getHeader().setVisible(false);
    ...
    bodyX.addControlListener(new ControlAdapter() {
      ...
      public void controlResized(ControlEvent e) {
        int width = 0;
        for (int i = 0; i < bodyX.getCount(); i++) {
          width += bodyX.getCellWidth(i) + bodyX.getLineWidth(i);
        }
        if (width > matrix.getClientArea().width) {
          // Tutaj jest sytuacja kiedy suma szeroko�ci kolumn wykracza poza client area
  ...
  private void resize() {
    ...
    System.out.println("client area size: " + matrix.getClientArea());
  ...
  public static void main(String[] args) {
    ...
    new GuiTable_Jacek(shell, SWT.RIGHT);

    shell.open();
    shell.setSize(264, 200);
