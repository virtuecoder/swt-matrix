...
/**
 * Native (not emulated) boolean editors.
 */
/*
 * The cell editor depends on the cell value.
 */
public class Snippet_0405 {
	...
	public Snippet_0405() throws IOException {
...
//				image = getImage(value);
		...
		Point size = button.computeSize(13, 13);
		Rectangle r = matrix.getBody().getCellBounds(0, 1);
		r.x = r.x + (r.width - size.x) / 2;
		r.width = size.x;
		r.height = size.y;
		button.setBounds(r);
		button.moveAbove(matrix);
